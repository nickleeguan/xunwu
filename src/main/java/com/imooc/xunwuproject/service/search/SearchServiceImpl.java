...
import com.imooc.xunwuproject.base.RentValueBlock;
import com.imooc.xunwuproject.entity.House;
...
import com.imooc.xunwuproject.repository.HouseDetailRepository;
import com.imooc.xunwuproject.repository.HouseRepository;;
import com.imooc.xunwuproject.repository.HouseTagRepository;
import com.imooc.xunwuproject.repository.SupportAddressRepository;
...
public class SearchServiceImpl implements ISearchService {
    ...
    private void createOrUpdateIndex(HouseIndexMessage message){
        ...
        String address = city.getCnName() + region.getEnName() + house.getStreet() + house.getDistrict()
        ...
        if (!success || !serviceResult.isSuccess()){
            ...
            logger.debug("Index success with houses: " + houseId);
        }
    }
    ...
    private void remove(Long houseId, int retry) {
        if (retry > HouseIndexMessage.MAX_RETRY){
            logger.error("retry remove times 3 for house:" + houseId + "please check it");
            return;
        }
        ...
        try {
            ...
            logger.error("Cannot encode json for:" + houseId);
        }
    }
    //更具索引新建数据到es
    @Override
    ...
    public ServiceMultiResult<Long> query(RentSearch rentSearch) {
        ...
        SearchRequestBuilder requestBuilder = this.esClient.prepareSearch(INDEX_NAME)
                ...
                .setFetchSource(HouseIndexKey.HOUSE_ID, null);//返回固定字段
    ...
    public ServiceMultiResult<Long> mapQuery(MapSearch mapSearch) {
        ...
        for (SearchHit hit : response.getHits()) {
            Long parse = Longs.tryParse(String.valueOf(hit.getSource().get(HouseIndexKey.HOUSE_ID)));
            houseIds.add(parse);
        }
